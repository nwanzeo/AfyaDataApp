...
public class SQLiteHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_FEEDBACK_TABLE = "CREATE TABLE "
                + TABLE_FEEDBACK + "("
                + KEY_ID + " INTEGER PRIMARY KEY," // and auto increment will be handled with
                + KEY_FEEDBACK_ID + " TEXT,"
                + KEY_FORM_ID + " TEXT,"
    ...
    public Feedback getFeedback(Long id) {
        ...
        Cursor cursor = db.query(TABLE_FEEDBACK, new String[] { KEY_ID,
                        KEY_FEEDBACK_ID, KEY_FORM_ID, KEY_INSTANCE_ID, KEY_MESSAGE, KEY_DATE_CREATED }, KEY_ID + "=?",
    ...
    public List<Feedback> getAllFeedback() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                feedback.setId(Integer.parseInt(cursor.getString(1)));//feedbackId
                feedback.setFormId(cursor.getString(2));//formId
                feedback.setInstanceId(cursor.getString(3));//instanceId
                feedback.setMessage(cursor.getString(4));//Message
                feedback.setDateCreated(cursor.getString(5));//CreatedDate
